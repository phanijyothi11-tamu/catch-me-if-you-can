...
def extract_all_data_from_newsmtsc_format_entry(entry):
    global bad_entry_counter  # allow modifying the global counter
    global good_entry_counter  # allow modifying the global counter
...
class AbsaDatasetLoader(object):
    ...
    def load_data(self, specific_split=None, shuffle=False, prob_shuffle=None):
        ...
        print(f"#######################Number of bad entries skipped: {bad_entry_counter}")
        print(f"#######################Number of good entries : {good_entry_counter}")
        return datasets
...
class AbsaDataCollator(object):
    ...
    def __call__(self, batch):
        ...
        for entry in batch:
            batch_tokens.append(entry[0][0])  # Access the token tensor
            sentiments.append(entry[-2])      # Sentiment label
            params.append(entry[-1])           # Params

        # Find the max sequence length in the batch
        max_seq_len = max(tensor.squeeze(0).size(0) for tensor in batch_tokens)

        # Pad all tokens to max_seq_len
        padded_batch_tokens = []
        for tensor in batch_tokens:
            tensor = tensor.squeeze(0)   # <---- SQUEEZE here
            pad_len = max_seq_len - tensor.size(0)
            if pad_len > 0:
                pad_tensor = torch.full((pad_len,), self.tokenizer_padding_id, dtype=torch.long)
                tensor = torch.cat([tensor, pad_tensor], dim=0)
            padded_batch_tokens.append(tensor)

        batch_tokens = torch.stack(padded_batch_tokens, dim=0)  # [batch_size, max_seq_len]

        attention_masks = (batch_tokens != self.tokenizer_padding_id).long()
        ...
        x_select = torch.zeros_like(batch_tokens)
