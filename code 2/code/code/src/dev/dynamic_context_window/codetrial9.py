...
with open("validation.jsonl", "r") as f:
    valid_data = [json.loads(line.strip()) for line in f]
...
class SubsentenceSentimentBiLSTMWithEntityAttention(nn.Module):
    def __init__(self, hidden_size=768, lstm_hidden_size=256, num_classes=3):
        ...
        # Token-level attention inside window
        self.token_attention = nn.Linear(lstm_hidden_size * 2, 1)

        # Entity-level attention across entities
        self.entity_attention = nn.Linear(lstm_hidden_size * 2, 1)

        # Final classifier
        self.fc = nn.Linear(lstm_hidden_size * 2, num_classes)

    def forward(self, padded_contexts):
        """
        padded_contexts: [num_entities, max_window_size, hidden_size]
        """
        # Pass the entire batch of entity windows through BiLSTM together
        lstm_out, _ = self.lstm(padded_contexts)  # [num_entities, max_window_size, 2*lstm_hidden_size]

        # Token-level attention over the padded dimension
        token_scores = self.token_attention(lstm_out).squeeze(-1)  # [num_entities, max_window_size]
        token_weights = F.softmax(token_scores, dim=1)  # [num_entities, max_window_size]

        # Weighted sum of LSTM outputs for each entity
        token_pooled = torch.bmm(token_weights.unsqueeze(1), lstm_out).squeeze(1)  # [num_entities, 2*lstm_hidden_size]

        # Entity-level attention
        entity_scores = self.entity_attention(token_pooled).squeeze(-1)  # [num_entities]
        entity_weights = F.softmax(entity_scores, dim=0)  # [num_entities]

        # Weighted sum of entity representations
        weighted_entities = (entity_weights.unsqueeze(-1) * token_pooled).sum(dim=0)  # [2*lstm_hidden_size]

        # Final classifier
        logits = self.fc(weighted_entities.unsqueeze(0))  # [1, num_classes]
        return logits
...
model_subsentiment = SubsentenceSentimentBiLSTMWithEntityAttention().to(device)
...
for epoch in range(epochs):
    ...
    for data in train_data:
        try:
            ...
            context_list = [e["vector"].squeeze(0) for e in entity_context_vectors]

            # Pad sequences so they have the same length
            padded_contexts = pad_sequence(context_list, batch_first=True)  # [num_entities, max_window_size, hidden_size]

            subsentiment_logits = model_subsentiment(padded_contexts)
  # squeeze extra batch dim
            # subsentiment_logits = model_subsentiment(context_list)
            ...
            global_logits = subsentiment_logits
    ...
    with torch.no_grad():
        for sample in valid_data:
            try:
                ...
                print(f"Skipping sample during validation due to error: {e}")

    val_accuracy = val_correct / val_total  # OUTSIDE no_grad
    print(f"Epoch {epoch+1}: Validation Accuracy = {val_accuracy:.4f}")

    # --- Early Stopping check (also OUTSIDE no_grad)
    if val_accuracy > best_val_accuracy:
        best_val_accuracy = val_accuracy
        patience_counter = 0
        torch.save({
            'model_state_dict': model_subsentiment.state_dict(),
...
model_subsentiment.load_state_dict(checkpoint['model_state_dict'])
