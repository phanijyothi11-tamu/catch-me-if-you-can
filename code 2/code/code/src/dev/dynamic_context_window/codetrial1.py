...
def preprocess_sample(data, window_size=10, alpha=0.25):
...
class SubsentenceSentimentHead(nn.Module):
    ...
    def forward(self, context_vectors):
        logits = self.fc(context_vectors)
        return logits
...
for epoch in range(epochs):
    ...
    for data in train_data:
        try:
            ...
            context_vectors = torch.stack([e["vector"] for e in entity_context_vectors])  # [num_entities, hidden]
...
for sample in tqdm(test_data):
    try:
        ...
        context_vectors = torch.stack([e["vector"] for e in entity_context_vectors])
        subsentiment_logits = model_subsentiment(context_vectors)
