...
class SubsentenceSentimentBiLSTM(nn.Module):
    def __init__(self, hidden_size=768, lstm_hidden_size=256, num_classes=3):
        ...
        self.attention = nn.Linear(lstm_hidden_size * 2, 1)  # Attention layer to score each token
    ...
    def forward(self, context_vectors_padded):
        ...
        attention_scores = self.attention(lstm_out).squeeze(-1)  # [batch_size, window_size]

        # Masking is optional if you have variable lengths; skip for now.

        # Apply softmax across window size
        attention_weights = F.softmax(attention_scores, dim=1)  # [batch_size, window_size]

        # Weighted sum of LSTM outputs
        pooled = torch.bmm(attention_weights.unsqueeze(1), lstm_out).squeeze(1)  # [batch_size, 2*lstm_hidden_size]
