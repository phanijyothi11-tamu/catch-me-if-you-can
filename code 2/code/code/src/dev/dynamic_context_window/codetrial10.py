...
class SubsentenceSentimentHead(nn.Module):
    def __init__(self, hidden_size=768, num_classes=3):
        ...
        nn.init.xavier_uniform_(self.fc.weight)  # (NEW) Weight initialization
...
torch.nn.utils.clip_grad_norm_(model_subsentiment.parameters(), max_norm=1.0)
...
for epoch in range(epochs):
    ...
    for data in train_data:
        try:
            ...
            context_list = [e["vector"] for e in entity_context_vectors]
            ...
            torch.nn.utils.clip_grad_norm_(model_subsentiment.parameters(), max_norm=1.0)  # Gradient clipping
    ...
    scheduler.step()  # (NEW) Step the LR scheduler after each epoch
