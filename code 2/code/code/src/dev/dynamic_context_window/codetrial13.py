...
from torch.optim import AdamW  # <- NEW correct import
...
model_name = "microsoft/deberta-v3-small"  # Better for English
...
for name, param in bert_model.named_parameters():
    if "pooler" in name or "LayerNorm" in name:
...
optimizer = AdamW([
    {"params": model.parameters(), "lr": 2e-4},    # your LSTM+Attention+FC head
    {"params": bert_model.parameters(), "lr": 1e-5}  # tiny learning rate for Pooler
], lr=2e-4)

train_steps_per_epoch = len(train_data)
num_training_steps = train_steps_per_epoch * 30
