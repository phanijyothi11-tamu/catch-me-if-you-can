...
class SentData(Dataset):
    def __init__(self, records, dom_id):
        ...
        for r in records:
            for t in r["targets"]:
                # Skip the entity_positions field
                self.items.append(
...
class DANN(nn.Module):
    ...
    def forward(self, ids, mask):
        # Get token representations from RoBERTa
        outputs = self.roberta(ids, attention_mask=mask)
        hidden_states = outputs.last_hidden_state  # (batch_size, seq_len, hidden_size)

        # Apply max-pooling to the entire sequence
        pooled = hidden_states.max(dim=1)[0]  # Max pooling across the sequence length
...
def run_epoch(model, dl, optim=None, sched=None):
    ...
    for b in tqdm(dl, leave=False):
        ...
        out_s, out_d = model(ids, mask)  # Removed entity_positions argument
...
for seed in SEEDS:
    ...
    for ep in range(1, MAX_EPOCHS+1):
        ...
        if val_f1 > best:
            best = val_f1; bad = 0
            torch.save(model.state_dict(), f"best_seedtdmn{seed}.pt")
        else:
            bad += 1
...
for seed in SEEDS:
    model.load_state_dict(torch.load(f"best_seedtdmn{seed}.pt"))
