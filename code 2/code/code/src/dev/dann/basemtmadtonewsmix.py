...
EARLY_PATIENCE = 3
MAX_LEN = 128  # XLM-RoBERTa max tokens
...
val_ds = SentData(mad_val, 0, use_mt=False)  # No MT for MAD validation
test_ds = SentData(news_train, 1, use_mt=False)  # No MT for News test
...
test_dl = DataLoader(test_ds, batch_size=VAL_BATCH)
...
class DANN(nn.Module):
    def __init__(self, lamb=1.0, n_sent=3, n_dom=2):
        super().__init__()
        self.xlmroberta = XLMRobertaModel.from_pretrained("xlm-roberta-base")  # Changed to XLM-RoBERTa
        h = self.xlmroberta.config.hidden_size
    ...
    def forward(self, ids, mask):
        pooled = self.xlmroberta(ids, attention_mask=mask).pooler_output
...
for seed in SEEDS:
    ...
    # Final model test on News test data
    print("Evaluating on News test.")
    loss, macro, report = run_epoch(model, test_dl)
...
loss, macro, report = run_epoch(model, test_dl)
print("Testing on News test data:")
