...
# Define Dataset: removing the target entity from the input
    ...
    def __init__(self, records, dom_id):
        ...
        for r in records:
            for t in r["targets"]:
                ...
                entity = t["mention"]
                # Remove the target entity from the sentence
                if entity:
                    sentence = sentence.replace(entity, "")
                self.items.append(
                    (sentence.strip(), t["polarity"], dom_id)
                )
...
for seed in SEEDS:
    ...
    for ep in range(1, MAX_EPOCHS+1):
        ...
        _, val_f1, _ = run_epoch(model, val_dl)
        print(f"[seed {seed}] Epoch {ep}/{MAX_EPOCHS}  val Macro-F1 = {val_f1:.3f}")
        if val_f1 > best:
            best = val_f1; bad = 0
            torch.save(model.state_dict(), f"best_seednm{seed}.pt")
        else:
            bad += 1
...
for seed in SEEDS:
    model.load_state_dict(torch.load(f"best_seednm{seed}.pt"))
