...
val_dl   = DataLoader(mt_ds, batch_size=VAL_BATCH)  # Validate on RW
...
class DANN(nn.Module):
    ...
    def forward(self, ids, mask):
        # Using last_hidden_state instead of pooler_output
        output = self.roberta(ids, attention_mask=mask)
        last_hidden_state = output.last_hidden_state
        
        # Max pooling over the sequence length dimension
        pooled = last_hidden_state.max(dim=1)[0]  # Max-pooling along the sequence dimension
...
for seed in SEEDS:
    ...
    for ep in range(1, MAX_EPOCHS+1):
        ...
        if val_f1 > best:
            best = val_f1; bad = 0
            torch.save(model.state_dict(), f"best_seedmtnewstomad{seed}.pt")
        else:
            bad += 1
...
for seed in SEEDS:
    model.load_state_dict(torch.load(f"best_seedmtnewstomad{seed}.pt"))
