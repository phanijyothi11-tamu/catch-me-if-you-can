...
from transformers import RobertaTokenizer, RobertaModel  # Changed to Roberta imports
...
WEIGHT_DECAY    = 0.01# ════════════════════════════════════════════════════════════════════════
# Domain‑Adversarial BERT for Target‑Dependent Sentiment (News -> MAD)
# ────────────────────────────────────────────────────────────────────────
!pip -q install transformers scikit-learn sentencepiece torchtext

# ─────────────────── 1. Imports & device ────────────────────────────────
from google.colab import drive; drive.mount('/content/drive')
import os, json, random, torch, torch.nn as nn
from torch.utils.data import Dataset, DataLoader, ConcatDataset
from transformers import RobertaTokenizer, RobertaModel  # Using RoBERTa
from torch.optim import AdamW
from sklearn.metrics import f1_score, classification_report
from tqdm.notebook import tqdm
import torchtext
...
for seed in SEEDS:
    ...
    for ep in range(1, MAX_EPOCHS + 1):
        ...
        _, val_f1, _ = run_epoch(model, val_dl)  # evaluate on MAD validation
