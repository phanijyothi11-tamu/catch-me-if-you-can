...
mad_test  = load_jsonl(f"{MAD}/test.jsonl")
...
class SentData(Dataset):
    def __init__(self, records, dom_id):
        ...
        for r in records:
            ...
            for t in r["targets"]:
                entity = t["mention"]
                # Remove the entity from the sentence
                sentence_wo_entity = sentence.replace(entity, "")
                self.items.append(
                    (sentence_wo_entity, t["polarity"], dom_id)
                )
...
val_dl   = DataLoader(rw_ds, batch_size=VAL_BATCH)
test_dl  = DataLoader(mad_ds, batch_size=VAL_BATCH)
...
for seed in SEEDS:
    ...
    for ep in range(1, MAX_EPOCHS+1):
        ...
        if val_f1 > best:
            best = val_f1; bad = 0
            torch.save(model.state_dict(), f"best_seednewstomad{seed}.pt")
        else:
            bad += 1
...
for seed in SEEDS:
    model.load_state_dict(torch.load(f"best_seednewstomad{seed}.pt"))
