...
class DANN(nn.Module):
    ...
    def forward(self, ids, mask):
        outputs = self.xlm_roberta(ids, attention_mask=mask)  # Using XLM-RoBERTa last_hidden_state
        last_hidden_state = outputs.last_hidden_state
        # Max-pooling over the sequence length (axis 1)
        pooled = torch.max(last_hidden_state, dim=1)[0]  # Max pooling over the sequence length (dimension 1)
