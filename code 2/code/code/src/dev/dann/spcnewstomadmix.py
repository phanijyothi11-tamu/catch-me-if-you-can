...
# Domain‑Adversarial BERT for Target‑Dependent Sentiment (Mixed Train on News and MAD-TSC, Test on MAD-TSC)
...
from transformers import RobertaTokenizer, RobertaModel  # Using RoBERTa instead of XLM-RoBERTa
...
# ─────────────────── 4. Dataset & loaders ──────────────────────────────
tok = RobertaTokenizer.from_pretrained("roberta-base")  # Using RoBERTa tokenizer
...
# Training on a mix of News and MAD-TSC
train_ds = ConcatDataset([
    SentData(news_train, 1),      # News as source domain
    SentData(mad_train, 0)        # MAD-TSC as target domain
])

# Testing on MAD-TSC validation dataset
val_ds   = SentData(mad_val,0)
mt_ds    = SentData(news_mt,1)
rw_ds    = SentData(news_rw,1)

train_dl = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True)
val_dl   = DataLoader(val_ds,   batch_size=VAL_BATCH)  # Evaluating on MAD-TSC validation set
...
class DANN(nn.Module):
    def __init__(self, lamb=1.0, n_sent=3, n_dom=2):
        super().__init__()
        self.roberta = RobertaModel.from_pretrained("roberta-base")  # Using RoBERTa model
...
for seed in SEEDS:
    ...
    for ep in range(1, MAX_EPOCHS+1):
        run_epoch(model, train_dl, optim, sched)            # train on the mix
        _, val_f1, _ = run_epoch(model, val_dl)             # evaluate using MAD-TSC validation set
