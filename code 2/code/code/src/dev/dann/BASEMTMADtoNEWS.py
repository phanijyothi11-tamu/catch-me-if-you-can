...
from transformers import XLMRobertaTokenizer, XLMRobertaModel  # Changed to XLM-RoBERTa
...
# Load MAD-TSC and NewsMTSC datasets
...
# ------------------- 4. Dataset & loaders ------------------------------
tok = XLMRobertaTokenizer.from_pretrained("xlm-roberta-base")  # Changed to XLM-RoBERTa tokenizer
label_map = {2:0, 4:1, 6:2}                    # neg, neu, pos

# Define Dataset for MAD-TSC (train and validation) and NewsMTSC (test)
class SentData(Dataset):
    def __init__(self, records, dom_id):
        self.items = []
        for r in records:
            for t in r["targets"]:
                # Replace entities in sentence (you can modify this part based on how you identify entities)
                sentence = r["sentence_normalized"]
                # Assuming `entity` is defined somewhere, here is how you could replace it
                for entity in r.get("entities", []):  # If entities are provided in each record
                    sentence = sentence.replace(entity, "")
                self.items.append(
                    (sentence, t["polarity"], dom_id)
                )
...
# Train and validation datasets on MAD-TSC
train_ds = SentData(mad_train, 0)
val_ds   = SentData(mad_val, 0)

# Test datasets on NewsMTSC (MT and Real)
mt_ds    = SentData(news_mt, 1)
rw_ds    = SentData(news_rw, 1)

# DataLoaders for MAD-TSC and NewsMTSC
...
class DANN(nn.Module):
    def __init__(self, lamb=1.0, n_sent=3, n_dom=2):
        super().__init__()
        self.xlm_roberta = XLMRobertaModel.from_pretrained("xlm-roberta-base")  # Changed to XLM-RoBERTa model
...
for seed in SEEDS:
    ...
    for ep in range(1, MAX_EPOCHS+1):
        ...
        if val_f1 > best:
            best = val_f1; bad = 0
            torch.save(model.state_dict(), f"best_seedmtmadtonews{seed}.pt")
        else:
            bad += 1
...
for seed in SEEDS:
    model.load_state_dict(torch.load(f"best_seedmtmadtonews{seed}.pt"))
    for name, dl in [("NewsMTSC-MT", mt_dl),
                     ("NewsMTSC-Real", rw_dl)]:
