...
#SBATCH --job-name=MAD-TSC-RO   # Sets the job name to AbaqusJob
...
#SBATCH --output=job.o%J  # Sends stdout and stderr to job.o[jobID]
...
mkdir -p "$DATA_SCRATCH/$UUID/MAD_TSC_ro"
...
rsync -avh ./data/MAD_TSC/original/ro/ "$DATA_SCRATCH/$UUID/MAD_TSC_ro/"

# === Log timing and status ===
start_time=$(date +%s)
echo "Starting test.py at $(date)..."
echo "[INFO] Training with SPC on Roman"
...
--dataset-config configs/datasets/MAD_TSC_ro.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/ro_spc \
