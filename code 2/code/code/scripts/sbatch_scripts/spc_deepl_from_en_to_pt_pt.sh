...
#SBATCH --job-name=spc_deepl_from_en_to_pt_pt
...
# Copy dataset
rsync -avh ./data/MAD_TSC/deepl/from_en_to_pt/ "$DATA_SCRATCH/$UUID/from_en_to_pt/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on deepl/from_en_to_pt with model neuralmind/bert-base-portuguese-cased"

# Launch training
tscbench finetune tsc \
-n spc_deepl_from_en_to_pt_pt \
...
--dataset-config configs/datasets/MAD_TSC_deepl_from_en_to_pt.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/deepl_from_en_to_pt_pt_spc \
