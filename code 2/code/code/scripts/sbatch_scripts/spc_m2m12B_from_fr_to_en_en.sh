...
#SBATCH --job-name=spc_m2m12B_from_fr_to_en_en
#SBATCH -A 132742115536
#SBATCH --time=12:00:00
#SBATCH --tasks=1
#SBATCH --ntasks-per-node=1
#SBATCH --mem=16G
#SBATCH --output=logs/job_TG_fr_to_en.o%J
...
mkdir -p "$DATA_SCRATCH/$UUID/from_fr_to_en"
mkdir -p $EXPERIMENT_DIR
mkdir -p $EXPERIMENT_SCRATCH

# Copy dataset
rsync -avh ./data/MAD_TSC/m2m12B/from_fr_to_en/ "$DATA_SCRATCH/$UUID/from_fr_to_en/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on m2m12B/from_fr_to_en with model roberta-base"

# Launch training
tscbench finetune tsc \
-n spc_m2m12B_from_fr_to_en_en \
-m "roberta-base" \
-t "roberta-base" \
--tsc-model-config configs/models/spc_model_default.json \
--dataset-config configs/datasets/MAD_TSC_m2m12B_from_fr_to_en.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/m2m12B_from_fr_to_en_en_spc \
