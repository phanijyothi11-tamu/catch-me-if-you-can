...
#SBATCH --job-name=spc_deepl_from_en_to_es_es
...
# Copy dataset
rsync -avh ./data/MAD_TSC/deepl/from_en_to_es/ "$DATA_SCRATCH/$UUID/from_en_to_es/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on deepl/from_en_to_es with model bertin-project/bertin-roberta-base-spanish"

# Launch training
tscbench finetune tsc \
-n spc_deepl_from_en_to_es_es \
...
--dataset-config configs/datasets/MAD_TSC_deepl_from_en_to_es.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/deepl_from_en_to_es_es_spc \
