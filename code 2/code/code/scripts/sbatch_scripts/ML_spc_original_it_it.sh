...
#SBATCH --job-name=ML_spc_original_it_it
...
mkdir -p "$DATA_SCRATCH/$UUID/MAD_TSC_it"
mkdir -p $EXPERIMENT_DIR
mkdir -p $EXPERIMENT_SCRATCH

# Copy dataset
rsync -avh ./data/MAD_TSC/original/it/ "$DATA_SCRATCH/$UUID/MAD_TSC_it/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on original/it with model xlm-roberta-base"

# Launch training
tscbench finetune tsc \
-n ML_spc_original_it_it \
-m "xlm-roberta-base" \
-t "xlm-roberta-base" \
--tsc-model-config configs/models/spc_model_default.json \
--dataset-config configs/datasets/MAD_TSC_it.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/ML_original_it_it_spc \
