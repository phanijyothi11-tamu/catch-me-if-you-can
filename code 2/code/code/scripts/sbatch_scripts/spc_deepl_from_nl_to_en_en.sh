...
#SBATCH --job-name=spc_deepl_from_nl_to_en_en
...
# Copy dataset
rsync -avh ./data/MAD_TSC/deepl/from_nl_to_en/ "$DATA_SCRATCH/$UUID/from_nl_to_en/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on deepl/from_nl_to_en with model roberta-base"

# Launch training
tscbench finetune tsc \
-n spc_deepl_from_nl_to_en_en \
-m "roberta-base" \
-t "roberta-base" \
--tsc-model-config configs/models/spc_model_default.json \
--dataset-config configs/datasets/MAD_TSC_deepl_from_nl_to_en.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/deepl_from_nl_to_en_en_spc \
