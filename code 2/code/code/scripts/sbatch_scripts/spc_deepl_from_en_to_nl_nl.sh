...
#SBATCH --job-name=spc_deepl_from_en_to_nl_nl
...
# Copy dataset
rsync -avh ./data/MAD_TSC/deepl/from_en_to_nl/ "$DATA_SCRATCH/$UUID/from_en_to_nl/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on deepl/from_en_to_nl with model pdelobelle/robbert-v2-dutch-base"

# Launch training
tscbench finetune tsc \
-n spc_deepl_from_en_to_nl_nl \
...
--dataset-config configs/datasets/MAD_TSC_deepl_from_en_to_nl.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/deepl_from_en_to_nl_nl_spc \
