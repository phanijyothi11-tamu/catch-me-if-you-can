...
#SBATCH --job-name=spc_m2m12B_from_en_to_it_it
...
mkdir -p "$DATA_SCRATCH/$UUID/from_en_to_it"
mkdir -p $EXPERIMENT_DIR
mkdir -p $EXPERIMENT_SCRATCH

# Copy dataset
rsync -avh ./data/MAD_TSC/m2m12B/from_en_to_it/ "$DATA_SCRATCH/$UUID/from_en_to_it/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on m2m12B/from_en_to_it with model dbmdz/bert-base-italian-uncased"

# Launch training
tscbench finetune tsc \
-n spc_m2m12B_from_en_to_it_it \
-m "dbmdz/bert-base-italian-uncased" \
-t "dbmdz/bert-base-italian-uncased" \
--tsc-model-config configs/models/spc_model_default.json \
--dataset-config configs/datasets/MAD_TSC_m2m12B_from_en_to_it.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/m2m12B_from_en_to_it_it_spc \
