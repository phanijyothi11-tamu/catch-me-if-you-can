...
#SBATCH --job-name=spc_deepl_from_en_to_it_it
...
# Copy dataset
rsync -avh ./data/MAD_TSC/deepl/from_en_to_it/ "$DATA_SCRATCH/$UUID/from_en_to_it/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on deepl/from_en_to_it with model dbmdz/bert-base-italian-uncased"

# Launch training
tscbench finetune tsc \
-n spc_deepl_from_en_to_it_it \
-m "dbmdz/bert-base-italian-uncased" \
-t "dbmdz/bert-base-italian-uncased" \
--tsc-model-config configs/models/spc_model_default.json \
--dataset-config configs/datasets/MAD_TSC_deepl_from_en_to_it.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/deepl_from_en_to_it_it_spc \
