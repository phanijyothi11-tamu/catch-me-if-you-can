...
#SBATCH --job-name=spc_deepl_from_en_to_fr_fr
...
# Copy dataset
rsync -avh ./data/MAD_TSC/deepl/from_en_to_fr/ "$DATA_SCRATCH/$UUID/from_en_to_fr/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on deepl/from_en_to_fr with model camembert-base"

# Launch training
tscbench finetune tsc \
-n spc_deepl_from_en_to_fr_fr \
-m "camembert-base" \
-t "camembert-base" \
--tsc-model-config configs/models/spc_model_default.json \
--dataset-config configs/datasets/MAD_TSC_deepl_from_en_to_fr.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/deepl_from_en_to_fr_fr_spc \
