...
#SBATCH --job-name=spc_deepl_from_en_to_ro_ro
...
# Copy dataset
rsync -avh ./data/MAD_TSC/deepl/from_en_to_ro/ "$DATA_SCRATCH/$UUID/from_en_to_ro/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on deepl/from_en_to_ro with model dumitrescustefan/bert-base-romanian-cased-v1"

# Launch training
tscbench finetune tsc \
-n spc_deepl_from_en_to_ro_ro \
...
--dataset-config configs/datasets/MAD_TSC_deepl_from_en_to_ro.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/deepl_from_en_to_ro_ro_spc \
