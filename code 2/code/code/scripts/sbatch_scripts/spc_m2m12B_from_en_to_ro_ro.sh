...
#SBATCH --job-name=spc_m2m12B_from_en_to_ro_ro
...
mkdir -p "$DATA_SCRATCH/$UUID/from_en_to_ro"
mkdir -p $EXPERIMENT_DIR
mkdir -p $EXPERIMENT_SCRATCH

# Copy dataset
rsync -avh ./data/MAD_TSC/m2m12B/from_en_to_ro/ "$DATA_SCRATCH/$UUID/from_en_to_ro/"

# Log starting time
start_time=$(date +%s)
echo "Starting training at $(date)..."
echo "[INFO] Training SPC model on m2m12B/from_en_to_ro with model dumitrescustefan/bert-base-romanian-cased-v1"

# Launch training
tscbench finetune tsc \
-n spc_m2m12B_from_en_to_ro_ro \
-m "dumitrescustefan/bert-base-romanian-cased-v1" \
-t "dumitrescustefan/bert-base-romanian-cased-v1" \
--tsc-model-config configs/models/spc_model_default.json \
--dataset-config configs/datasets/MAD_TSC_m2m12B_from_en_to_ro.json \
--gpu-pl-config configs/lightning/default_lightning_config.json \
--sub-path-final-folder run_outputs/m2m12B_from_en_to_ro_ro_spc \
